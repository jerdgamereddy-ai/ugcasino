...
- **Race Game House-Edge Safety** (Greyhound, Horse4, Horse-JS): each `/bet` endpoint pre-checks via `computeForceLose(gameType, userId, bet*maxOdds)` (combines `lockedLossMap` + RTP-cap probe using bet-amount × max odds) and returns `forceLose`; wrappers postMessage `set_win_chance` (dog/horse4 mutate `WIN_OCCURRENCE` global before `generateFinalRank`) or `set_force_lose` (horse-js suppresses payout). Because some engines (e.g. horse4 `gotoGame` → `generateFinalRank`) resolve the race before the bet response arrives, the immediate-round safety net is on the `/win` side: when `applyHouseEdgeForWin` blocks a positive intended win, `/win` returns `blocked:true`; the wrapper then postMessages `sync_balance` to overwrite the iframe's local `s_iCurMoney`/`_balance` and shows a destructive toast ("Round result voided") so the player isn't confused by a visual win that didn't credit.
