...
- **Per-Manager Game Overrides**: Each manager can override `winChance` and `payoutMultiplier` per game (table `manager_game_overrides`) for their own players only. All other game-settings fields (house edge, bet limits, special multipliers like dog odds / plinko / wheel) remain admin-global. Server-side helper `getEffectiveGameSettings(userId, gameType)` walks the createdBy chain to the nearest manager ancestor and merges that manager's override on top of the admin-global row. Manager UI lives in ManagerDashboard "Game Settings" tab; blank inputs inherit global, and a Clear button removes the override row.
