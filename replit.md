...
### Game Mechanics
- **Classic Slots** (`/classic-slots`): 5-reel, 20-payline HTML5 slot machine at `/games/classic-slots/index.html`; postMessage protocol (`game_ready` → send balance, `bet_placed` → POST bet, `save_score` → calculate win delta); configurable win chance and payout multiplier; backend at `/api/games/classic-slots/settings|bet|win`
...
- **Fish Joy** (`/fishhunt`): Canvas-based cannon fish game (`/games/fish-new/`). Players fire at fish swimming across screen. Bet selector (100/200/500/1K/2K/5K/10K UGX per shot) shown at bottom; fish odds panel on right shows each fish type with multiplier; win = bet × fishMultiplier; 12 fish types with configurable multipliers; postMessage protocol (`init_balance` → coins, `init_settings` → fishOdds, `bet`/`win` → API calls); backend `/api/games/fishjoy/bet`, `/api/games/fishjoy/win`, `/api/games/fishjoy/settings`; fish odds stored in extraSettings JSON
- **Greyhound Racing** (`/dog-racing`): Imported greyhound race HTML5 game with UGX currency, postMessage protocol; 6-dog odds configurable by admin (Psycho, All Saturdays, The Norman, T-Rex, Nice Tuft, Baloo); defaults [3.7, 5.5, 2.2, 11.75, 17.25, 8.75]; settings at `/api/games/dog-racing/settings`; bet/win backend endpoints at `/api/games/dog-racing/bet` and `/api/games/dog-racing/win`
...
- **Quick Horse Race** (`/horse-racing`): Custom 4-horse race JS game with UGX UI, 500 UGX bet steps, per-horse Win/Place/Show odds configurable by admin (stored in extraSettings JSON as `odds`/`placeOdds`/`showOdds`), admin-configurable max laps; in-game Bet Type selector (Win=1st, Place=top 2, Show=top 3); payout resolves from the matching odds array once enough horses cross the finish line; backend settings at `/api/games/horse-js/settings`, bet/win at `/api/games/horse-js/bet` and `/api/games/horse-js/win`
- **House Edge**: Administrators can adjust win probabilities per game type and coinflip payout multiplier through the Game Control panel; horse-js, dog-racing, and horse4 each have a per-runner Win/Place/Show odds grid (Win=1st, Place=top 2, Show=top 3); roulette has per-type odds panel; Fish Joy has 12-fish multiplier odds panel (FishJoyOddsCard). For dog-racing and horse4 the imported HTML5 game still resolves only Win in-game; Place/Show are stored on the server and forwarded via postMessage for upcoming side-bet support.
...
### Reporting System
- **Unified Reports API** (`GET /api/reports`): Server-side time filtering with `from`/`to` ISO timestamps, `managerId` filter, and `gameType` filter (e.g., `classic-slots`, `roulette`, `horse-js`, etc.)
...
- **Per-Game Breakdown**: Response includes `perGame` array (bets, wins, profit, plays, RTP) for every detected game type plus a `gameTypes` lookup; UI shows a "Per-Game Breakdown" table with per-row Filter button; Reports page Filters bar has a "Filter by Game" dropdown
- **Game Detection**: `detectGameType()` maps transaction descriptions (e.g., "Classic Slots: ...", "Greyhound Racing: ...", "Roulette: ...") to game keys server-side
