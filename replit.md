...
- **Broadcast System**: Hierarchical messaging (Admin to all, Super Manager to managers, Manager to players) with public broadcasts, customizable styling, duration, and dismissibility. The "Sent Broadcasts" table in `BroadcastSender.tsx` shows a Live/Disabled status badge plus per-row **Disable** (sets `expiresAt` to one second in the past so the marquee stops everywhere immediately) and **Delete** buttons. Backed by `POST /api/broadcasts/:id/disable` and `DELETE /api/broadcasts/:id` — admins can disable/delete any broadcast, managers/super-managers can only manage broadcasts they sent themselves.
- **Hi-Lo real playing cards** (`HiLoGame.tsx`): rewritten to render full 52-card-deck visuals on a felt background — proper suits (♠♥♦♣) with red/black coloring, rank labels in opposing corners, classic pip layouts for number cards (2–10), centered glyph for Aces, gold-bordered face panels for J/Q/K, and a UG-monogrammed blue card-back during the reveal flip. Suit is randomized per card (cosmetic only, server still drives win logic). Reveal flip uses framer-motion `rotateY` with a request-token pattern + cleanup ref so stale callbacks from cancelled rounds can't overwrite a new round's state, and Cash Out is disabled while a flip is mid-flight.
- **Royal Dice 3D** (`DiceGame.tsx`): replaced the flat `lucide-react` dice icon with a real CSS 3D cube — `transform-style: preserve-3d` + 6 ivory/gold beveled faces positioned via `rotate{X,Y}(...) translateZ(80px)`, parent uses `perspective: 900` so the cube has depth. While waiting for the server result the cube tumbles via `requestAnimationFrame`; on settle it transitions (`cubic-bezier(0.2,0.9,0.2,1.05)`, 1.4s) to a `REST_ROTATION` orientation that brings the rolled face to the front, with extra full turns added so it always looks like it kept spinning. Sits on a green felt arena with an inner-shadow rim and a soft drop-shadow ellipse beneath the cube. Uses request-token + cleanup-on-unmount pattern so post-unmount/abandoned-roll timers cannot fire `setState`.
...
- **Automated Game Schedules** (`game_schedules` table + admin "Auto Schedule" tab): Admin defines rules like *"between 18:00–22:00 on weekends, set Roulette winChance=25% and payoutMultiplier=1.8"*. Each row carries `gameType`, `startTime`/`endTime` (HH:MM, 24h, server-local; overnight wrap supported), `daysOfWeek` (CSV of 0=Sun..6=Sat), optional `winChancePct`/`payoutMultiplier`, and `enabled`. A 60s `setInterval` (`applySchedules`, guarded by both a `(global as any).__gameScheduleTick` re-registration guard and a `scheduleTickRunning` re-entrancy flag) finds active rules for the current minute, picks the last-defined rule per game, and writes the configured values into `game_settings` via existing `updateGameSettings` / `updateGamePayoutMultiplier`. CRUD endpoints `GET/POST/PATCH/DELETE /api/admin/game-schedules` (admin-only). UI lives in `ScheduleManager` inside `AdminDashboard.tsx`.
- **Per-Game Pie Chart in Reports** (`Reports.tsx`): The Per-Game Breakdown card now also renders a doughnut `<PieChart>` of bets-per-game using a 16-shade `PIE_PALETTE` (gold/amber → magenta → violet → blue → teal → emerald → lime), so each of the 12+ games gets a clearly distinct slice with percentage labels.
