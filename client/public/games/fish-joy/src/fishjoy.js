	...
	setTimeout(function()
	{
		if(window.parent && window.parent !== window) {
			...
			window.addEventListener('message', function(e) {
				if(!e.data || e.data.type !== 'init_balance') return;
				_parentBalance = e.data.balance;
				if(!_gameStarted) {
					_gameStarted = true;
					game.load();
				}
...
{
	var coin = (typeof _parentBalance !== "undefined" && _parentBalance !== null) ? _parentBalance : (Number(this.params.coin) || 10000);
